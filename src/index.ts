...
export default {
  async fetch(
    ...
    const addressParam = q
      .split("?")
      .find((param) => param.startsWith("address="));
      ...
      return makeRpcResponse(
        {
          error: {
            code: -32000,
            message: "Has likely been subsidized before.",
          },
        },
        400,
      );
    ...
    const userBal = await relayer.call("eth_getBalance", [
      ethAddress,
      "latest",
    ]);
    ...
    const relayBal = await relayer.call("eth_getBalance", [
      relay_.address,
      "latest",
    ]);
